...
function TrendBadge({ value }: { value: number }) {
  ...
  return (
    <span
      ...
      style={{ color: positive ? C.green : C.rose }}
    >
      {positive ? (
...
export default function TelemetryDashboardPage() {
  ...
  const miniCharts = useMemo<MiniChartCardProps[]>(() => {
    ...
    const lastPeak = cd.reduce(
      (max, c) => (c.total > max.total ? c : max),
      cd[0],
    );
    const lastErrorBurst = [...cd].reverse().find((c) => c.errors > 0);
    const lastLatencySpike = cd.reduce(
      (max, c) => (c.latency > max.latency ? c : max),
      cd[0],
    );
    ...
    return [
      {
        title: "System Status",
        legend: [
          { label: "Service Health", color: C.orange },
          { label: "Availability", color: C.green },
        ],
        data: cd.map((c) => ({
          name: c.name,
          a: c.total,
          b: Math.max(0, c.total - c.errors),
        })),
        footer:
          chartData.length > 0
            ? `Last Incident: ${lastErrorBurst?.name ?? "None"}`
            : "Awaiting data…",
      },
      {
        title: "Throughput",
        legend: [
          { label: "Requests per Minute", color: C.orange },
          { label: "Success Rate", color: C.green },
        ],
        data: cd.map((c) => ({
          name: c.name,
          a: c.total,
        ...
        footer:
          chartData.length > 0
            ? `Last Peak: ${lastPeak?.name ?? "N/A"}`
            : "Awaiting data…",
      },
      {
        title: "Latency",
        legend: [
          { label: "P50–P95 Latency", color: C.orange },
          { label: "Error Latency", color: C.green },
        ],
        data: cd.map((c) => ({
          name: c.name,
          a: c.latency,
          b: c.errors > 0 ? c.latency * 1.5 : 0,
        })),
        footer:
          chartData.length > 0
            ? `Last Spike: ${lastLatencySpike?.name ?? "N/A"}`
            : "Awaiting data…",
      },
      {
        title: "Error Rate",
        legend: [
          { label: "Application Errors", color: C.orange },
          { label: "System Failures", color: C.green },
        ],
        data: cd.map((c) => ({
          name: c.name,
          a: c.clientErrors,
        ...
        footer:
          chartData.length > 0
            ? `Last Error Burst: ${lastErrorBurst?.name ?? "None"}`
            : "Awaiting data…",
      },
      {
        title: "Active Users",
        legend: [
          { label: "Authenticated", color: C.orange },
          { label: "Anonymous", color: C.green },
        ],
        data: cd.map((c) => ({ name: c.name, a: c.total, b: c.errors })),
        footer:
          chartData.length > 0
            ? `${data?.summary?.unique_users ?? 0} unique users today`
            : "Awaiting data…",
      },
      {
        title: "Recent Activity",
        legend: [
          { label: "Requests", color: C.orange },
          { label: "Latency Trend", color: C.green },
        ],
        data: cd.map((c) => ({ name: c.name, a: c.total, b: c.latency })),
        footer:
          chartData.length > 0
            ? `Avg: ${data?.summary?.avg_response_ms?.toFixed(0) ?? 0}ms`
            : "Awaiting data…",
      },
    ];
  }, [chartData, data]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="System Status"
          value={
            s?.error_rate !== undefined && s.error_rate < 1
              ? `${(100 - s.error_rate).toFixed(2)}%`
              : "99.00%"
          }
          sublabel="Uptime"
            ...
            <tbody>
              {(data?.endpoint_health?.length ?? 0) > 0 ? (
                ...
                <tr>
                  ...
                  >
                    No endpoint data available yet. Make some API requests to
                    see metrics.
                  </td>
                </tr>
              )}
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      background: `${err.status_code >= 500 ? C.rose : C.amber}15`,
                    }}
                  >
                    <AlertTriangle
                      className="h-3.5 w-3.5"
                      style={{
                        color: err.status_code >= 500 ? C.rose : C.amber,
                      }}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium truncate">
                      <span className="font-mono">{err.status_code}</span>{" "}
                      {err.method} {err.path}
                    </p>
                    <p className="text-[10px] text-muted-foreground truncate">
                      {err.user__email || "Anonymous"} ·{" "}
                      {err.response_time_ms.toFixed(0)}ms
                    </p>
                  </div>
