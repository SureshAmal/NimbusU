    ...
    useEffect(() => {
        async function fetchGradesData() {
            try {
                ...
                setGrades(Array.isArray(gradesRes.data) ? gradesRes.data : (gradesRes.data as any).results ?? []);
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Skeleton className="h-24 w-full" style={{ borderRadius: "var(--radius-lg)" }} />
                    <Skeleton className="h-24 w-full" style={{ borderRadius: "var(--radius-lg)" }} />
                    <Skeleton className="h-24 w-full" style={{ borderRadius: "var(--radius-lg)" }} />
                </div>
                <Skeleton className="h-[400px] w-full" style={{ borderRadius: "var(--radius-lg)" }} />
            </div>
        );
    }
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        View your course grades and cumulative GPA.
                    </p>
                </div>
                <Button
                    variant="outline"
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <div className="p-6 rounded-xl border bg-card text-card-foreground shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h2 className="text-3xl font-bold">{gpaData?.cgpa ? gpaData.cgpa.toFixed(2) : "N/A"}</h2>
                    </div>
                </div>
                ...
                <div className="p-6 rounded-xl border bg-card text-card-foreground shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h2 className="text-3xl font-bold">{gpaData?.total_credits || 0}</h2>
                    </div>
                </div>
            ...
            <div className="rounded-xl border shadow-sm bg-card overflow-hidden">
                ...
                {grades.length === 0 ? (
                    <div className="p-8 text-center text-muted-foreground flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm">Your grades will appear here once they are published by faculty.</p>
                    </div>
                ) : (
                            ...
                            <TableBody>
                                {grades.map((grade) => (
                                    <TableRow key={grade.id}>
                                        ...
                                        <TableCell>{grade.semester_name || grade.semester || "Current"}</TableCell>
                                        <TableCell className="text-center">{grade.credits_earned}</TableCell>
                                        <TableCell className="text-center">{grade.grade_points}</TableCell>
                                        <TableCell className="text-center">
                                            <span className="font-bold text-lg">{grade.grade_letter}</span>
                                        </TableCell>
                                        <TableCell className="text-right">
                                            {grade.is_pass ? (
                                                <Badge variant="secondary" className="bg-emerald-500/15 text-emerald-600 hover:bg-emerald-500/25">
                                                    Pass
                                                </Badge>
                                            ) : (
