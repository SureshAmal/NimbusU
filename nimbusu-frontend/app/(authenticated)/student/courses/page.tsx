...
export default function StudentCoursesPage() {
    ...
    const enrolledOfferingIds = new Set(enrollments.map(e => e.course_offering));
    const openOfferings = availableOfferings.filter(o => !enrolledOfferingIds.has(o.id));

    return (
        <div className="space-y-6 max-w-6xl mx-auto pb-10">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">Courses & Enrollment</h1>
                <p className="text-sm text-muted-foreground mt-1">Manage your current enrollments and register for new courses</p>
            </div>
                ...
                <TabsContent value="my-courses" className="space-y-4 outline-none">
                    {enrollments.length === 0 ? (
                        <div className="py-16 text-center text-muted-foreground border rounded-xl border-dashed">
                            ...
                            <p className="text-sm mt-1">Switch to the Available Courses tab to enroll.</p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex flex-col h-full gap-3 p-5 rounded-xl border border-transparent bg-card shadow-sm hover:border-border hover:shadow-md transition-all cursor-pointer">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="font-semibold truncate leading-tight">{e.course_name}</p>
                                                <p className="text-xs text-muted-foreground mt-0.5">{e.status === "waitlisted" ? "Waitlisted" : "Active Enrollment"}</p>
                                            </div>
                                        </div>
                                        <div className="mt-auto pt-2 flex items-center justify-between">
                                            <Badge variant={e.status === "active" ? "default" : e.status === "waitlisted" ? "outline" : "secondary"}>
                                                {e.status}
                                            </Badge>
                                            <span className="text-[10px] text-muted-foreground">Enrolled {new Date(e.enrolled_at).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </Link>
                            ))}
                ...
                <TabsContent value="enroll" className="space-y-4 outline-none">
                    <div className="bg-muted/30 border rounded-xl p-4 mb-6">
                        ...
                        <ul className="text-sm text-muted-foreground mt-2 space-y-1 list-disc list-inside">
                            ...
                            <li>You must have passed all prerequisites to enroll in advanced courses.</li>
                    ...
                    {openOfferings.length === 0 ? (
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {openOfferings.map((o) => {
                                ...
                                return (
                                    <div key={o.id} className="flex flex-col h-full gap-4 p-5 rounded-xl border bg-card shadow-sm">
                                        <div>
                                            <div className="flex justify-between items-start gap-2 mb-1">
                                                <h3 className="font-semibold leading-tight">{o.course_name}</h3>
                                                <Badge variant="outline" className="shrink-0">{o.course_code}</Badge>
                                            </div>
                                            <p className="text-sm text-muted-foreground">{o.faculty_name} • Section {o.section}</p>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between text-sm mt-auto pt-4 border-t">
                                            ...
                                            >
                                                {enrollingMap[o.id] ? "Enrolling..." : (isFull ? "Join Waitlist" : "Enroll")}
                                            </Button>
                                        </div>
                                    </div>
                                );
