...
export default function StudentMessagesPage() {
  ...
  useChatSocket((newMessage) => {
    setMessages((prev) => {
      // Avoid adding duplicate messages (e.g. if we just sent it and it bounded back, though receiver logic prevents this)
  ...
  useEffect(() => {
    ...
    usersService
      .list({ role: "faculty" })
      .then(({ data }) => setFaculty(data.results ?? []))
      .catch(() => {});
  }, [composeOpen]);

  const inbox = useMemo(
    () => messages.filter((m) => m.receiver === user?.id),
    [messages, user],
  );
  const sent = useMemo(
    () => messages.filter((m) => m.sender === user?.id),
    [messages, user],
  );

  async function handleSend(e: React.FormEvent) {
    e.preventDefault();
    if (!receiver || !body.trim()) {
      toast.error("Fill all required fields");
      return;
    }
    setSending(true);
    try {
      await messagesService.send({ receiver, subject, body });
      toast.success("Sent!");
  ...
  async function openMessage(msg: Message) {
    try {
      const { data } = await messagesService.get(msg.id);
      ...
      <div className="space-y-0.5">
        {items.map((m) => {
          ...
          return (
            ...
            >
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full shrink-0 ${!m.is_read && isIncoming ? "bg-primary/10" : "bg-muted"}`}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p
                    className={`text-sm ${!m.is_read && isIncoming ? "font-semibold" : "font-medium"}`}
                  >
                    {isIncoming ? m.sender_name : m.receiver_name}
                  </p>
                  {!m.is_read && isIncoming && (
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="inbox">
        ...
        <TabsContent value="inbox" className="mt-3">
          <MessageList items={inbox} emptyText="No messages received." />
        </TabsContent>
        <TabsContent value="sent" className="mt-3">
          <MessageList items={sent} emptyText="No messages sent." />
        </TabsContent>
      </Tabs>
